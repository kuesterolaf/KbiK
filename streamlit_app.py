...
v_name = st.sidebar.text_input("Vorname:", key="vname_input").strip()
n_name = st.sidebar.text_input("Nachname:", key="nname_input").strip()
t_liste = ["-- Bitte wählen --", "Eintracht Vorleser", "FC Bücherwurm", "Rasenball Lesen", "SpVgg Buchdeckel"]
team = st.sidebar.selectbox("Dein Team:", t_liste, key="team_select")
...
if v_name and n_name and team != "-- Bitte wählen --":
    ...
    # Kategorie AUSSERHALB des Forms für Interaktivität
    kat = st.sidebar.radio("Was meldest du?", ["Lesezeit (Minuten)", "Buch abgeschlossen 🏆"], key="kat_radio")
    
    with st.sidebar.form("entry_form"):
        if kat == "Lesezeit (Minuten)":
            auswahl = st.selectbox("Dauer:", ["30 min gelesen (2 Pkt)", "60 min gelesen (4 Pkt)"], key="min_sel")
            p = 2 if "30" in auswahl else 4
        else:
            auswahl = st.selectbox("Umfang:", ["Buch bis 100 S. (5 Pkt)", "Buch bis 200 S. (10 Pkt)", "Buch über 200 S. (15 Pkt)"], key="buch_sel")
...
with c2:
    ...
    if not df.empty:
        # ABSOLUTER DATENSCHUTZ: Keine Namen in der Liste
        disp = df.iloc[::-1][["Datum", "Team", "Details", "Punkte"]].head(10)
        st.dataframe(disp, use_container_width=True, hide_index=True)

st.markdown("---")
st.info("ℹ️ **Datenschutz:** Es werden keine Spielernamen veröffentlicht. Nur die Team-Leistung ist sichtbar!")
