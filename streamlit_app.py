...
if v_name and n_name and team != "-- Bitte wählen --":
    ...
    # KATEGORIE AUSSERHALB DES FORMULARS (für sofortige Reaktion)
...
with c2:
    ...
    if not df.empty:
        # Hier werden Vornamen und Nachnamen komplett weggelassen!
        disp = df.iloc[::-1][["Datum", "Team", "Details", "Punkte"]].head(10)
        st.dataframe(disp, use_container_width=True, hide_index=True)

st.markdown("---")
st.info("ℹ️ **Datenschutz:** In dieser Liste werden keine Spielernamen veröffentlicht. Nur die Team-Leistung ist sichtbar!")
